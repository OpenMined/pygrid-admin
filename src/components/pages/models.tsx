...
function TagsShowcase({id, tags, onRemove}: {id: string | number; tags: string[]; onRemove: (e: string) => void}) {
  return (
    <div className="space-y-1">
      <p className="font-medium ml-1 text-sm text-gray-700">Tags</p>
      <div className="flex space-x-4">
        ...
        {tags.map(tag => (
          <div key={`${id}-${tag}`} className="flex">
            ...
            {tags.length > 1 && (
              <button
                className="cursor-pointer px-2 rounded-r-md text-white items-center flex bg-red-400 hover:bg-red-800"
...
function AddNewTag({onAdd}) {
  ...
  return (
    <div className="space-y-1">
      <label htmlFor="add" className="font-medium ml-1 text-sm text-gray-700">
        Add a tag
      </label>
      <div className="flex space-x-4">
        <div className="flex">
...
function ModelEditFormButtons({onCancel}) {
  return (
    <div className="w-full text-right space-x-6">
...
function EditPanel(model: Model) {
  ...
  return (
    <div className="py-6 px-4 text-sm border-t border-gray-200 bg-blueGray-100 space-y-6">
        ...
        <Accordion.Item key={model.id}>
          <div className="w-full">
            ...
            <div className="text-sm text-gray-700 space-y-2">
              <p className="mt-4 overflow-ellipsis line-clamp-4 pr-12 text-sm text-gray-700">{model.description}</p>
